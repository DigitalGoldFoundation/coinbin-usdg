      ...
      f = (e.util = {
        ...
        endian: function (a) {
          if (a.constructor == Number)
            return (f.rotl(a, 8) & 16711935) | (f.rotl(a, 24) & 4278255360);
          for (var b = 0; b < a.length; b++) a[b] = f.endian(a[b]);
          return a;
        },
        ...
        bytesToBase64: function (a) {
          for (var b = [], c = 0; c < a.length; c += 3)
            for (
              var d = (a[c] << 16) | (a[c + 1] << 8) | a[c + 2], e = 0;
              e < 4;
              e++
            )
              c * 8 + e * 6 <= a.length * 8
        ...
        base64ToBytes: function (a) {
          for (
            var a = a.replace(/[^A-Z0-9+\/]/gi, ""), b = [], c = 0, d = 0;
            c < a.length;
            d = ++c % 4
          )
            d != 0 &&
